...
final class CronJobRepositoryTest extends TestCase
{
    ...
    public function testPresetMatchesFilterValue(): void
    {
        ...
        $this->assertTrue('minimal' === $minimal->getPreset());
        $this->assertFalse('minimal' === $custom->getPreset());
    }

    public function testManagedPresetFilterKeepsCustomJobs(): void
    {
        $managed = ['minimal', 'standard', 'full'];
        $custom = new CronJob();
        $custom->setPreset('custom');
        $minimal = new CronJob();
        $minimal->setPreset('minimal');
        $standard = new CronJob();
        $standard->setPreset('standard');

        $this->assertContains($minimal->getPreset(), $managed);
        $this->assertContains($standard->getPreset(), $managed);
        $this->assertNotContains($custom->getPreset(), $managed);
    }
